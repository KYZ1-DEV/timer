...
<body class="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex items-center justify-center p-4 text-white font-sans antialiased overflow-hidden">
    ...
    <!-- Audio Senyap untuk menjaga tab tetap hidup di background -->
    <!-- Base64 dari file WAV kosong berdurasi sepersekian detik -->
    ...
    <main class="glass-panel w-full max-w-md rounded-[2.5rem] p-8 relative z-10 flex flex-col items-center transition-all duration-500">
        ...
        <div id="pomodoro-indicator" class="flex flex-col items-center mb-4 h-12 transition-all">
            ...
            <div class="flex gap-1.5" id="session-dots"></div>
        </div>
    ...
    <script>
        ...
        function playAlertSound() {
            ...
            const playTone = (freq, type, duration, startTime) => {
                ...
                gain.gain.setValueAtTime(0.5, audioCtx.currentTime + startTime + duration - 0.05); 
                gain.gain.linearRampToValueAtTime(0.001, audioCtx.currentTime + startTime + duration); 
                osc.connect(gain);
                gain.connect(audioCtx.destination);
                osc.start(audioCtx.currentTime + startTime);
                osc.stop(audioCtx.currentTime + startTime + duration);
            };
            // 5 kali beep (nit) selama total 5 detik
            for (let i = 0; i < 8; i++) {
                playTone(800, 'sine', 0.5, i * 1.0); 
            }
        }

        // Inline Web Worker untuk background tick yang lebih stabil
        ...
        // Screen Wake Lock API untuk mencegah layar mati otomatis jika dibuka
        let wakeLock = null;
        ...
        const app = {
            ...
            init() {
                ...
                // Menerima detak dari Web Worker
                bgWorker.onmessage = () => {
                    this.syncTimer();
                };

                // Sync saat tab dibuka kembali (Visibility API)
            ...
            loadState() {
                ...
                if (saved) {
                    try { this.state = JSON.parse(saved); } catch (e) { console.error("Gagal load state"); }
                }
            },
            ...
            startTimer(isNew = true) {
                ...
                // MEMAINKAN AUDIO SENYAP AGAR BACKGROUND TETAP HIDUP
                this.els.silentAudio.play().catch(e => console.log("Silent audio blocked by browser, requires interaction"));
            ...
            updateUI(fastUpdate = false) {
                ...
                if (this.state.mode === 'pomodoro') {
                    ...
                    for (let i = 1; i <= 4; i++) {
                        ...
                        if (i < this.state.sessionCount) dot.className = "w-2.5 h-2.5 rounded-full bg-white";
                        else if (i === this.state.sessionCount) dot.className = "w-2.5 h-2.5 rounded-full bg-white shadow-[0_0_8px_white] animate-pulse";
                        else dot.className = "w-2.5 h-2.5 rounded-full bg-white/20";
